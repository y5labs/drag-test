...
const components = {
  ...
  schedule: (schedules, def, clip, tz) =>
    between(schedules, schedules[def.schedule], clip, tz)
}
...
const operations = {
  or: (a, b, tz) => {
    ...
    while (aindex < a.length && bindex < b.length) {
      ...
      if (isBefore(a[aindex].start, b[bindex].start)) {
        result.push(a[aindex])
        aindex++
        continue
      }
      if (isAfter(a[aindex].start, b[bindex].start)) {
  ...
  not: (a, b, tz) => {
    ...
    while (process.length > 0 && bindex < b.length) {
      ...
      if (isBefore(process[0].start, b[bindex].start)) {
        result.push(process[0])
        process.shift()
        continue
      }
      if (isAfter(process[0].start, b[bindex].start)) {
        bindex++
        continue
      }
    }
...
const between = (schedules, defs, clip, tz) => {
  let result = []
  for (let def of defs) {
